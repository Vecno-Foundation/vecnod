...
impl RpcCoreService {
    ...
    fn sanity_check_storage_mass(&self, block: Block) {
        // [Starlight]: warn non updated miners to upgrade their rpc flow before Starlight activation
        if self.config.starlight_activation.is_active(block.header.daa_score) {
            return;
        }
...
You must upgrade your miner flow to propagate the mass field correctly prior to the Starlight hardfork activation. 
Failure to do so will result in your blocks being considered invalid when Starlight activates.",
                            block.hash(),
                            tx.id()
                        );
...
impl RpcApi for RpcCoreService {
    async fn submit_block_call(
        ...
        if !request.allow_non_daa_blocks {
            ...
            //
            // [Starlight]: switch to the larger duration only after a full window with the new duration is reached post activation
...
NOTE: This error usually indicates an RPC conversion error between the node and the miner. This is likely to reflect using a NON-SUPPORTED miner.",
    ...
    async fn estimate_network_hashes_per_second_call(
        ...
        if request.window_size as u64 > self.config.pruning_depth().lower_bound() {
            return Err(RpcError::WindowSizeExceedingPruningDepth(request.window_size, self.config.current_pruning_depth));
        }
