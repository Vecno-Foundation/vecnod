...
pub struct Params {
    ...
    pub current_difficulty_window_size: usize,
    ...
    pub current_max_block_parents: u8,
    pub current_mergeset_size_limit: u64,
    pub current_merge_depth: u64,
    pub current_finality_depth: u64,
    pub current_pruning_depth: u64,

    pub coinbase_payload_script_public_key_max_len: u8,
    pub max_coinbase_payload_len: usize,

    pub current_max_tx_inputs: usize,
    pub current_max_tx_outputs: usize,
    pub current_max_signature_script_len: usize,
    pub current_max_script_public_key_len: usize,
    ...
    pub current_coinbase_maturity: u64,
...
impl Params {
    ...
    pub fn difficulty_window_size(&self) -> ForkedParam<usize> {
        ForkedParam::new(
            self.current_difficulty_window_size,
    ...
    pub fn target_time_per_block(&self) -> ForkedParam<u64> {
        ForkedParam::new(self.current_target_time_per_block, self.starlight.target_time_per_block, self.starlight_activation)
    }
    ...
    pub fn bps(&self) -> ForkedParam<u64> {
        ForkedParam::new(
            1000 / self.current_target_time_per_block,
    ...
    pub fn ghostdag_k(&self) -> ForkedParam<KType> {
        ForkedParam::new(self.current_ghostdag_k, self.starlight.ghostdag_k, self.starlight_activation)
    }

    pub fn max_block_parents(&self) -> ForkedParam<u8> {
        ForkedParam::new(self.current_max_block_parents, self.starlight.max_block_parents, self.starlight_activation)
    }

    pub fn mergeset_size_limit(&self) -> ForkedParam<u64> {
        ForkedParam::new(self.current_mergeset_size_limit, self.starlight.mergeset_size_limit, self.starlight_activation)
    }

    pub fn merge_depth(&self) -> ForkedParam<u64> {
        ForkedParam::new(self.current_merge_depth, self.starlight.merge_depth, self.starlight_activation)
    }

    pub fn finality_depth(&self) -> ForkedParam<u64> {
        ForkedParam::new(self.current_finality_depth, self.starlight.finality_depth, self.starlight_activation)
    }

    pub fn pruning_depth(&self) -> ForkedParam<u64> {
        ForkedParam::new(self.current_pruning_depth, self.starlight.pruning_depth, self.starlight_activation)
    }

    pub fn coinbase_maturity(&self) -> ForkedParam<u64> {
        ForkedParam::new(self.current_coinbase_maturity, self.starlight.coinbase_maturity, self.starlight_activation)
    }

    pub fn finality_duration_in_milliseconds(&self) -> ForkedParam<u64> {
        ForkedParam::new(
            self.current_target_time_per_block * self.current_finality_depth,
    ...
    pub fn difficulty_window_duration_in_block_units(&self) -> ForkedParam<u64> {
        ForkedParam::new(
            self.current_difficulty_window_size as u64,
    ...
    pub fn expected_difficulty_window_duration_in_milliseconds(&self) -> ForkedParam<u64> {
        ForkedParam::new(
            self.current_target_time_per_block * self.current_difficulty_window_size as u64,
    ...
    pub fn anticone_finalization_depth(&self) -> ForkedParam<u64> {
        let prior_anticone_finalization_depth = self.current_finality_depth
            + self.current_merge_depth
            + 4 * self.current_mergeset_size_limit * self.current_ghostdag_k as u64
            + 2 * self.current_ghostdag_k as u64
            + 2;
        ...
        // not finalized.
        ForkedParam::new(
            min(self.current_pruning_depth, prior_anticone_finalization_depth),
    ...
    pub fn max_tx_inputs(&self) -> ForkedParam<usize> {
        ForkedParam::new(self.current_max_tx_inputs, self.starlight.max_tx_inputs, self.starlight_activation)
    }

    pub fn max_tx_outputs(&self) -> ForkedParam<usize> {
        ForkedParam::new(self.current_max_tx_outputs, self.starlight.max_tx_outputs, self.starlight_activation)
    }

    pub fn max_signature_script_len(&self) -> ForkedParam<usize> {
        ForkedParam::new(self.current_max_signature_script_len, self.starlight.max_signature_script_len, self.starlight_activation)
    }

    pub fn max_script_public_key_len(&self) -> ForkedParam<usize> {
        ForkedParam::new(self.current_max_script_public_key_len, self.starlight.max_script_public_key_len, self.starlight_activation)
    }
...
pub const MAINNET_PARAMS: Params = Params {
    peers: &["159.198.42.49:7111","209.74.87.79:7111","159.198.43.32:7111", "89.162.117.198:7111"],
    net: NetworkId::new(NetworkType::Mainnet),
    genesis: GENESIS,
    timestamp_deviation_tolerance: TIMESTAMP_DEVIATION_TOLERANCE,
    max_difficulty_target: MAX_DIFFICULTY_TARGET,
    max_difficulty_target_f64: MAX_DIFFICULTY_TARGET_AS_F64,
    current_difficulty_window_size: LEGACY_DIFFICULTY_WINDOW_SIZE,
    min_difficulty_window_size: MIN_DIFFICULTY_WINDOW_SIZE,

    //
    // ~~~~~~~~~~~~~~~~~~ BPS dependent constants ~~~~~~~~~~~~~~~~~~
    //
    current_ghostdag_k: OneBps::ghostdag_k(),
    current_target_time_per_block: OneBps::target_time_per_block(),
    current_max_block_parents: OneBps::max_block_parents(),
    current_mergeset_size_limit: OneBps::mergeset_size_limit(),
    current_merge_depth: OneBps::merge_depth_bound(),
    current_finality_depth: OneBps::finality_depth(),
    current_pruning_depth: OneBps::pruning_depth(),
    current_coinbase_maturity: OneBps::coinbase_maturity(),

    coinbase_payload_script_public_key_max_len: 150,
    max_coinbase_payload_len: 204,
    current_max_tx_inputs: 1000,
    current_max_tx_outputs: 1000,
    current_max_signature_script_len: 10_000,
    current_max_script_public_key_len: 10_000,
    mass_per_tx_byte: 1,
...
pub const TESTNET_PARAMS: Params = Params {
    ...
    current_ghostdag_k: OneBps::ghostdag_k(),
    current_target_time_per_block: OneBps::target_time_per_block(),
    current_max_block_parents: if OneBps::max_block_parents() > 64 { OneBps::max_block_parents() } else { 64 },
    current_mergeset_size_limit: OneBps::mergeset_size_limit(),
    current_merge_depth: OneBps::merge_depth_bound(),
    current_finality_depth: OneBps::finality_depth(),
    current_pruning_depth: OneBps::pruning_depth(),
