...
impl TestContext {
    ...
    pub fn build_block_template_row(&mut self, nonces: impl Iterator<Item = usize>) -> &mut Self {
        for nonce in nonces {
            self.simulated_time += self.consensus.params().current_target_time_per_block;
...
async fn antichain_merge_test() {
    let config = ConfigBuilder::new(MAINNET_PARAMS)
        ...
        .edit_consensus_params(|p| {
            p.current_max_block_parents = 4;
            p.current_mergeset_size_limit = 10;
        })
        .build();
...
async fn double_search_disqualified_test() {
    ...
    let config = ConfigBuilder::new(MAINNET_PARAMS)
        ...
        .edit_consensus_params(|p| {
            p.current_max_block_parents = 4;
            p.current_mergeset_size_limit = 10;
            p.min_difficulty_window_size = p.current_difficulty_window_size;
        })
        .build();
    let mut ctx = TestContext::new(TestConsensus::new(&config));
