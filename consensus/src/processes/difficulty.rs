...
use vecno_core::{info, log::STARLIGHT_KEYWORD};
...
#[derive(Clone)]
struct StarlightLogger {
    steps: Arc<AtomicU8>,
}

impl StarlightLogger {
    fn new() -> Self {
    ...
    pub fn report_activation_progress(&self, step: u8) -> bool {
        if self.steps.compare_exchange(step, step + 1, AtomicOrdering::SeqCst, AtomicOrdering::SeqCst).is_ok() {
            match step {
                Self::ACTIVATE => {
                    // TODO (Starlight): finalize mainnet ascii art
                    info!(target: STARLIGHT_KEYWORD,
                        r#""#
                    );
                    info!(target: STARLIGHT_KEYWORD, "[Starlight] Accelerating block rate")
                }
                Self::DYNAMIC => {}
                Self::FULL => {}
                _ => {}
...
pub struct SampledDifficultyManager<T: HeaderStoreReader, U: GhostdagStoreReader> {
    ...
    current_target_time_per_block: u64,
    target_time_per_block: u64,
    starlight_activation: ForkActivation,
    starlight_logger: StarlightLogger,
}
...
impl<T: HeaderStoreReader, U: GhostdagStoreReader> SampledDifficultyManager<T, U> {
    ...
    pub fn new(
        ...
        Self {
            ...
            current_target_time_per_block,
            target_time_per_block,
            starlight_activation,
            starlight_logger: StarlightLogger::new(),
        }
    }
    ...
    pub(crate) fn starlight_activated(&self, selected_parent: Hash) -> bool {
        let sp_daa_score = self.headers_store.get_daa_score(selected_parent).unwrap();
        self.starlight_activation.is_active(sp_daa_score)
    }
    ...
    pub fn calculate_difficulty_bits(&self, window: &BlockWindowHeap, ghostdag_data: &GhostdagData) -> u32 {
        ...
        // Until there are enough blocks for a valid calculation the difficulty should remain constant.
        //
        // [Starlight]: post activation special case -- first activated blocks which do not have
        ...
        if difficulty_blocks.len() < self.min_difficulty_window_size {
            ...
            // Check if the selected parent itself is already post starlight activation (by checking the DAA score
            // of its selected parent). We ruled out genesis, so we can safely assume the grandparent exists
            if self.starlight_activated(self.ghostdag_store.get_selected_parent(selected_parent).unwrap()) {
                ...
                let scaled_target = target * self.current_target_time_per_block / self.target_time_per_block;
                let scaled_bits = Uint256::try_from(scaled_target.min(self.max_difficulty_target)).unwrap().compact_target_bits();

                if self.starlight_logger.report_activation_progress(StarlightLogger::ACTIVATE) {
                    info!(target: STARLIGHT_KEYWORD, "[Starlight] Block target time change: {} -> {} milliseconds", self.current_target_time_per_block, self.target_time_per_block);
                    info!(target: STARLIGHT_KEYWORD, "[Starlight] Difficulty change: {} -> {} ", difficulty_desc(target), difficulty_desc(scaled_target));
                }
        ...
        if difficulty_blocks_len + 1 < self.difficulty_window_size as u64 {
            if self.starlight_logger.report_activation_progress(StarlightLogger::DYNAMIC) {
                info!(target: STARLIGHT_KEYWORD,
                    "[Starlight] Dynamic DAA reactivated, scaling the difficulty by the measured/expected duration ratio: \n\t\t\t\t\t\t  {} -> {} (measured duration: {}, expected duration: {}, ratio {:.4})",
            ...
            if CoinFlip::default().flip() {
                info!(target: STARLIGHT_KEYWORD,
                    "[Starlight] DAA window increasing post activation: {} (target: {})",
