...
impl TransactionValidator {
    ...
    fn check_coinbase_in_isolation(&self, tx: &Transaction) -> TxResult<()> {
        ...
        /*
        [Starlight]: moved this specific check to body_validation_in_context since it depends on fork activation
    ...
    fn check_transaction_outputs_count(&self, tx: &Transaction) -> TxResult<()> {
        ...
        // [Starlight]: keep the check here over the upper limit. Add a tight check to in_header_context validation
...
mod tests {
    ...
    fn validate_tx_in_isolation_test() {
        ...
        params.current_max_tx_inputs = 10;
        params.current_max_tx_outputs = 15;
        let tv = TransactionValidator::new_for_tests(
            params.current_max_tx_inputs,
            params.current_max_tx_outputs,
            params.current_max_signature_script_len,
            params.current_max_script_public_key_len,
            params.coinbase_payload_script_public_key_max_len,
            params.current_coinbase_maturity,
            Default::default(),
        );
        ...
        let mut tx = valid_tx.clone();
        tx.inputs = (0..params.current_max_tx_inputs + 1).map(|_| valid_tx.inputs[0].clone()).collect();
        assert_match!(tv.validate_tx_in_isolation(&tx), Err(TxRuleError::TooManyInputs(_, _)));

        let mut tx = valid_tx.clone();
        tx.inputs[0].signature_script = vec![0; params.current_max_signature_script_len + 1];
        assert_match!(tv.validate_tx_in_isolation(&tx), Err(TxRuleError::TooBigSignatureScript(_, _)));

        let mut tx = valid_tx.clone();
        tx.outputs = (0..params.current_max_tx_outputs + 1).map(|_| valid_tx.outputs[0].clone()).collect();
        assert_match!(tv.validate_tx_in_isolation(&tx), Err(TxRuleError::TooManyOutputs(_, _)));

        let mut tx = valid_tx.clone();
        tx.outputs[0].script_public_key = ScriptPublicKey::new(0, scriptvec![0u8; params.current_max_script_public_key_len + 1]);
